...
class TestRunning(QRunnable):
    ...
    def run(self):
        ...
        def rbfn_funct_aco(input_vector, solution):
            f_x = solution[0]  # theta
            for j in range(self.neurl_num):
                gaussian = gaussian_funct_aco(
        ...
        def gaussian_funct_aco(jth_neurl, v_x, v_m, o):
            temp = 0
            # means = np.array(
        ...
        while(not car.intersection(map_line)):
            ...
            output = rbfn_funct_aco(np.array(list4d), self.pocket_particle)
