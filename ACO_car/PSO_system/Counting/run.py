...
# DIT IS RUN_ACO BITCHES
    def run(self):
        ...
        for times in range(self.iteration_times):
            ...
            colony = ACO_system.ant_colony_for_continuous_domains.ACOr()
            ranges = []
            ...
            colony.set_cost(self.adaptation_funct_aco(0))
            colony.set_variables(1 + 3 * self.neurl_num, ranges)
            ...
            # self.signals.iteration.emit("Means: {}".format(solution[1:self.neurl_num]))
            # self.signals.iteration.emit("Weight: {}".format(solution[self.neurl_num:))
            # self.signals.iteration.emit("SD: {}".format(self.pocket_particle.sd))
        self.signals.iteration.emit("-------------PSO training finished---------------")
        print(solution)
    ...
    def adaptation_funct_aco(self, index):
        e_n = 0
        # [y_n - F(x_n)]^2
        for idx, expected_output in enumerate(self.train_data.wheel_angle):
    ...
    def rbfn_funct_aco(self, input_vector, idx):
        f_x = self.particles[idx].theta[0]  # theta
        for j in range(self.neurl_num):
            gaussian = self.gaussian_funct(
                j, input_vector, self.particles[idx].means, self.particles[idx].sd[j])
